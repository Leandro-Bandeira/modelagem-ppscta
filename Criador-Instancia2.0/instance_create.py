...
def main():
    ...
    similarity_data = open_similarity(path_similarity)  # Dados de similaridade
    ...
    # Criação da instancia para inviablizar pesos para trabalhos
    # do mesmo orientador, caso forem o mesmo, temos -1 como peso
    # Além disso, precisamos criar uma matriz orientadores X Trabalhos
    # contendo suas similaridades, vamos analisar qual a melhor matriz
    # similaridade de determinado orientador
    # E usá-la como o peso para o algoritmo, muito importante
    # perceber que os indices dos dois ARQUIVOS estão equiparados
    # ou seja a similaridade do projeto 0 equivale ao projeto 0.
    ...
    while (i < len(projetos_data)):
        ...
        if projetos_data[i]['Orientador:'] not in orientadores_visitados:
            ...
            media = 0
            # Se tivermos situação em que ele só tiver um projeto
            # então ele não entra no segundo for, como os indices estão iguais
            # da similaridade e do projeto a similaridade de indice
            # 0 representa as similaridades do mesmo projeto 0
            ...
            for similarity in similarity_data:
                ...
                data_similarity = similarity["similaridade"].rstrip(
                        ).lstrip().split()

                # Se encontrarmos o mesmo orientador
                # daquele fixdo em i na minha similaridade
                if projetos_data[indice]["Orientador:"] == projetos_data[i][
                        "Orientador:"]:
            ...
            # Nosso arquivo estará dividido em em uma linha indice
            # pego do autor correspondente aos projetos e seus beneficios

            avg_similarity = similarity_data[indice_maior_media][
                    "similaridade"].rstrip().lstrip().split()
            for j, similarity in enumerate(avg_similarity):
                # Se o orientador da linha for o mesmo da coluna
                # então -1 para indicar que o trabalho é inviável
                # Cso ele não for inviável, vamos normalizar as similaridades
                # Como analises indicam que 0.1 já indica pelo menos mesma
                # área e sub temos os seguintes valores para similaridade
                similarity_str = 0
